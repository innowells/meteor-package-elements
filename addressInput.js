...
function getAddr(name, ens, callback) {
  ...
  ens.resolver(node, function(err, resolverAddress) {
    if (!err && resolverAddress != 0 && resolverAddress != '0x') {
...
Template.dapp_addressInput.onCreated(function() {
  ...
  chain3.mc.getSyncing(function(err, syncing) {
    if (!err && !syncing) {
      // cannot use ENS while syncing
      chain3.mc.getCode(ensAddress, function(err, code) {
...
Template.dapp_addressInput.helpers({
  ...
  'address': function() {
    ...
    return (_.isString(address) && chain3.isAddress(address)) ? '0x' + address.replace('0x', '') : false;
  },
  /**
...
Template.dapp_addressInput.events({
  ...
  'input input, keyup input': function(e, template) {
    ...
    if (chain3.isAddress(value) || _.isEmpty(value)) {
      TemplateVar.set('isValid', true);

      if (!_.isEmpty(value)) {
        TemplateVar.set('value', '0x' + value.replace('0x', ''));
        TemplateVar.set('isChecksum', chain3.isChecksumAddress(value));
        ...
        if (TemplateVar.get('ensAvailable')) {
          ...
          getName(value, ens, function(name) {
            ...
            getAddr(name, ens, function(addr) {
              ...
              TemplateVar.set(template, 'value', chain3.toChecksumAddress(addr));
              e.currentTarget.value = chain3.toChecksumAddress(addr);
            });
          });
        }
      } else {
