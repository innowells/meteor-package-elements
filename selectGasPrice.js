...
@method calculateGasInSha
@return {Number}
*/
var calculateGasInSha = function(template, gas, gasPrice, returnGasPrice){
    ...
    // divide and multiply to round it to the nearest billion sha (1 gsha)
...
Template['dapp_selectGasPrice'].helpers({
    ...
    */
    'fee': function(){
        if(_.isFinite(TemplateVar.get('feeMultiplicator')) && _.isFinite(this.gas)) {
            ...
            // set the value
            TemplateVar.set('gasInSha', calculateGasInSha(template, this.gas, this.gasPrice).floor().toString(10));
            TemplateVar.set('gasPrice', calculateGasInSha(template, this.gas, this.gasPrice, true).floor().toString(10));

            // return the fee
            return McTools.formatBalance(calculateGasInSha(template, this.gas, this.gasPrice).toString(10), '0,0.[000000000000000000]', this.unit);
        }
    },
    /**
    ...
    */
    'unit': function(){
        var unit = this.unit || McTools.getUnit();
        if(unit)
